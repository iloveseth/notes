...
var tableView = cc.Class({
    ...
    _addCellsToView: function (isreload,emax) {
        if(isreload){
                for (var index = 0; index <= this._maxCellIndex; index++) {
                    // let xt = this._maxCellIndex - emax;
                    // if((index+1) <= xt){
                    //     this._addCell(index);
                    //     continue;
                    // }
            ...
            cc.log(this._maxCellIndex);
        }
    },

    _addCellsToViewByFrame: function(){
        this.emax = this._maxCellIndex + 1;
        this.startFrameLoad = true;
        if(this.startFrameLoad && this.content.childrenCount < this.emax){
            this._addCell(this.content.childrenCount);
        }
        if(this.startFrameLoad && this.content.childrenCount >= this.emax){
            this.startFrameLoad = false;
        }
    },
    ...
    _initTableView: function (clear,refresh) {
        if (this._cellPool) {
            this.clear();
        }
        ...
        this._changePageNum(this._initPage - this._page);

        var x = this._view.width;
        var y = this._view.height;
        x = (this._page - 1) * x;
        y = (this._page - 1) * y;
        this.scrollToOffset({ x: x, y: y });
        this._lastOffset = this.getScrollOffset();
        if (this._minCellIndex == 0 || clear) {
            this._minCellIndex = this._initPage - 1;
        }
        this._maxCellIndex = this._minCellIndex + this._cellCount - 1;

        if(refresh){
            this._addCellsToViewByFrame();
        }
        else{
    ...
    initTableView: function (paramCount, data, initpage = 1, clear = true,refresh = false) {
        ...
        // }
        this._initTableView(clear,refresh);
    },
    //更新内容 不重置位置   待优化 暂时先用
    reloadTableView: function (paramCount, data, needReload) {
        if (paramCount != this._paramCount) {
            if(paramCount > 4 && needReload){
                // 多个reload 减少渲染
                this._paramCount = paramCount;
                this._data = data;
                this._count = paramCount;
                this._cellCount = Math.ceil(this._view.height / this._cellSize.height) + 1;
                if (this._cellCount > this._count){
                    this._cellCount = this._count;
                }
                this.content.height = this._count * this._cellSize.height;
                let emax = this._maxCellIndex;
                this._maxCellIndex = this._minCellIndex + this._cellCount - 1;
                this._addCellsToView(true,emax);
            }else{
                let offset = this.getScrollOffset();
                this.initTableView(paramCount, data, this._initPage, false);
    ...
    scrollToBottom: function (timeInSecond, attenuated) {
        let gap = this._maxCellIndex + this._cellCount;
        let len = this._cellCount;
        let eMin = this._minCellIndex;
        let eMax = this._maxCellIndex;
        if(gap < this._count){
            let idx = this._count - 1 - (this._cellCount-1)*2;
            for (var i = eMin; i < len; i++) {
               var node = this.content.getChildByTag(i);
               node.y =  node.y - node.height*(idx - i);
               this._setCellAttr(node, idx);
               idx ++;
            }
            this._minCellIndex = this._count - 1 - (this._cellCount-1)*2;
            this._maxCellIndex = this._minCellIndex + this._cellCount - 1;
        }
    ...
    scrollToPage: function (page, withAnima = true) {
        ...
        var time = withAnima ? 0.3 * Math.abs(page - this._page) : 0.1;
                    ...
                    do {
                        ...
                        if (nodeBox.xMax <= viewBox.xMin) {
                            ...
                            if (nodeBox.xMax + (this._maxCellIndex - this._minCellIndex + 1) * node.width > viewBox.xMin
                                || this.content.getChildByTag(this._maxCellIndex) == null ) {
                    ...
                    do {
                        ...
                        if (nodeBox.xMin >= viewBox.xMax) {
                            ...
                            if (nodeBox.xMin - (this._maxCellIndex - this._minCellIndex + 1) * node.width < viewBox.xMax
                                || this.content.getChildByTag(this._minCellIndex) == null) {
                ...
                if (this._maxCellIndex < this._count - 1) {
                    ...
                    do {
                        ...
                        if (nodeBox.yMin >= viewBox.yMax) {
                            let child = this.content.getChildByTag(this._maxCellIndex);
                            node.y = _.get(child, 'y', 0) - node.height;
                            this._minCellIndex++;
                            this._maxCellIndex++;
                           
                            // 当拉取的长度较大的时候 不应该和viewBox的位置坐比较 而是要和MaxCellIndex的node位置作比较
                            // 相当于第一个位置上移1000长度，yMin-1000 ViewBox的yMax保持不变，maxcell-minCell*height是永远固定的
                            // 就是显示区域的宽度 所以移动足够长的时候 不会新建cell
...
cc.tableViewNew = module.exports = tableView;